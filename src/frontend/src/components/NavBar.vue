...
const props = defineProps(['view', 'user', 'logoutCurrentUser']);
</script>
...
<template>
  <nav class="bg-slate-400 text-2xl flex align-baseline">
    ...
    <div class="text-2xl ml-auto p-2 group relative">
      <Avatar class="h-10 bg-white" :username="props.user.name" />
      <div class="invisible group-hover:visible absolute top-full right-0 bg-slate-400 flex flex-col gap-1 px-2 py-1 rounded-bl-md">
        <router-link class="hover:underline" :to="{ name: 'Account', params: { userName: props.user.name }}">Profile</router-link>
        <button class="hover:underline" @click="props.logoutCurrentUser">Logout</button>
      </div>
    </div>
  </nav>
</template>
